...
<template>
    <div class="flex flex-col w-1/4 gap-y-2 pt-9">
        ...
        <div class="flex flex-col gap-2">
            ...
            <div class="">
                <select name="" id="" class="p-2 border-2 rounded-md w-full text-sm
                                bg-white outline-none focus:ring-2 focus:border-blue-600">
                    <option v-for="category in eventCategories" :key="category.id"
                        :value="category.eventCategoryName">{{category.eventCategoryName}}</option>
                </select>
            </div>
        </div>
